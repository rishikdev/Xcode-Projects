...
//    @AppStorage("isAppLocked") var isAppLocked: Bool = true
    ...
    private let myNotesContainer: NSPersistentCloudKitContainer
    ...
    init() {
        ...
        myNotesContainer.loadPersistentStores { (description, error) in
            if let error = error {
    ...
    func authenticate() {
        ...
        if(context.canEvaluatePolicy(.deviceOwnerAuthentication, error: &error)) {
            ...
            context.evaluatePolicy(.deviceOwnerAuthentication, localizedReason: touchIDReason) {  success, authenticationError in
